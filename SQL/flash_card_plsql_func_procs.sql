...
--DML insert
INSERT INTO flash_cards VALUES (7, 'this is 4?', 'I should show this in java');
INSERT INTO flash_cards (fc_id, fc_question) VALUES(2, 'what is java?');

INSERT INTO flash_cards (fc_question, fc_answer) VALUES('what is your favorite color?', '42');
COMMIT;
--DML select
...
--Procendural Language / SQL is oracles impl of SQL
--PL/SQL sequences
...
--allowed to write PL/SQL
  IF :new.fc_id IS NULL THEN
    --embedded SQL
    SELECT fc_seq.nextval INTO :new.fc_id FROM DUAL;--SELECT requires a FROM statement so we used a fake table called dual
  ...
  END IF;
END;
/--A break for functions triggers and procedures
