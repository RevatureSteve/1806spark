...
public class AppService {
	...
	public Users login(Users usersEmail) {
		System.out.println("[LOG]---Starting--- Service getUserByEmail for: " + usersEmail + " information");
		...
		// 		No - then it is null
		System.out.println("[LOG]---calling dao---Service getUserByEmail");
		...
		System.out.println("[LOG]---dao returned---Service getUserByEmail returned: " + dbEmail);
		
		// logic to compare user's email to email in Database
		if (dbEmail != null) {
			System.out.println("[LOG]---dbEmail null check---Service getUserByEmail() status: success");
			// compare user's email to dbEmail's password
			if (dbEmail.getPassword().equals(usersEmail.getPassword())) {
				System.out.println("[LOG]---compare password---Service getUserByEmail() password: success");
				// returning dbEmail to Users, not usersEmail
				return dbEmail;
			}
		}
		
		System.out.println("[LOG]---ending---Service getUserByEmail password: fail");
		return null;
	}
