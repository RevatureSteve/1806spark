...
public class ReimbursementDao {
	...
	public List<Reimbursement> getReimbursementByEmployeeId(int id) {
		...
		try (Connection conn = ConnectionUtil.getConnection();) {
			...
			String sql = "SELECT * FROM reimbursement r LEFT JOIN rq_status rs ON r.rq_status_id = rs.rq_status_id LEFT JOIN rq_type rt "
					+ "ON r.rq_type_id = rt.rq_type_id LEFT JOIN users u ON r.emp_u_id = u.u_id LEFT JOIN users u ON r.mgr_u_id = u.u_id WHERE emp_u_id = ? ORDER BY r_id";
	...
	public Reimbursement getReimbursementByRId(int rId) {
		...
		try (Connection conn = ConnectionUtil.getConnection();) {
			...
			String sql = "SELECT * FROM reimbursement r LEFT JOIN rq_status rs ON r.rq_status_id = rs.rq_status_id LEFT JOIN rq_type rt "
					+ "ON r.rq_type_id = rt.rq_type_id LEFT JOIN users u ON r.emp_u_id = u.u_id LEFT JOIN users u ON r.mgr_u_id = u.u_id WHERE r_id = ?";
