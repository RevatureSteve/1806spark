...
public class UserDao {
	...
	public User getUserByEmail(String email) {
		...
		try (Connection conn = SetConnectionPropertiesUtil.getConnection();){
			...
			if(rs.next()) {
				user = new User(rs.getInt("u_id"), rs.getString("email"), rs.getString("pass_word"), rs.getString("fname"), rs.getString("lname"), rs.getInt("pos_id"),rs.getString("pos_type"));
			}
	...
	public List<User> getAllUsers() {
		...
		try (Connection conn = SetConnectionPropertiesUtil.getConnection();){
			...
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
				User user = new User(rs.getInt(1), rs.getString("email"), rs.getString("pass_word"), rs.getString("fname"), rs.getString("lname"), rs.getInt("pos_id"),rs.getString("pos_type"));
				users.add(user);
			}
		} catch (IOException | SQLException e) {
			e.printStackTrace();
