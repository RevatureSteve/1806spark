...
app.listen(3000, function() {  // local host is 3000
...
app.use("/home", function(req, resp, next){
    resp.sendFile('C:\\Users\\lolitsrhys\\my_git_repos\\1806spark\\tampa_gems\\index.html');
})
...
app.use('/person/:id', function(req, resp, next){
    ...
    if(req.params.id > 50) {
        resp.json({
            "response": "your ide is really high",
            "ID": req.params.id
        })
    } else {
        resp.json({
...
/*  
    example for request body param s- POST Request
