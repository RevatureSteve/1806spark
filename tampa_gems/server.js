...
app.use("/home", function(req, resp, next){
    resp.sendFile('C:\\Users\\Alex Moraga\\my_git_repos\\1806spark\\tampa_gems\\index.html');
    // next(); don't need as this functino is sending the response back, not the next function
})


/*
    example for url params - GET Request
        query string ?key=value e.g. ?name=alex&age=150
...
app.use("/person/:id", function(req, resp, next){
    ...
    if(req.params.id > 50) {
        ...
        resp.json({
            ...
            "ID": req.params.id,
            "movies":["batman","superman", "avengers", "Spiderman"]
    });
}
    next();
})
